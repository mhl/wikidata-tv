...
def report(episodes):
            ...
            if arbitrary_episode.episodes_in_season:
                ...
                if arbitrary_episode.episodes_in_season != n_episodes_found:
                    report_items.append(
                        (
                            False,
                            '''The number of episodes actually found in season {season_item}
                               ({n_found}) was different from the number of episodes
                               suggested by the 'number of episodes' (P1113) statement
                               ({n_expected})'''.format(
                ...
                fmt = "Season {season_item} was missing the 'number of episodes' (P1113) statement"
                report_items.append((False, fmt.format(season_item=season_item)))
        else:
            fmt = 'There were no episodes at all found in season {season_item}!'
            report_items.append((False, fmt.format(season_item=season_item)))
    ...
    if len(episodes_without_an_episode_number):
        report_items.append(
            (
                False,
                '''{n} episodes were missing an episode number, which should be
                   specified as a 'series ordinal' (P1545) qualifier to the
                   'series' (P179) statement linking the episode to the series'''.format(
                    n=len(episodes_without_an_episode_number))
            )
        )
    return report_items
...
def report_extra_queries(query_service, series_item):
    ...
    if number_of_seasons is not None:
        if number_of_seasons == len(values):
            ...
            report_items.append(
                (
                    False,
                    '''The number of seasons actually found ({found}) didn't
                       match the 'number of seasons' (P2437) value {expected}'''.format(
                        found=len(values), expected=number_of_seasons)
                )
            )
    if not values:
        report_items.append(
            (
                False,
                '''No seasons were found at all - they should have a 'series'
                   (P179) relationship to {0}'''.format(series_item)
            )
        )
    ...
    for season in values:
        ...
        if 'seasonNumber' not in season:
            report_items.append(
                (
                    False,
                    '''No 'season ordinal' (P1545) qualifier was found for the
                       'series' (P179) statement for season {0}'''.format(season_item)
                )
            )
        if 'episodesInSeason' in season:
            season_to_number_of_episodes[season_item] = int(season['episodesInSeason'])
        else:
            fmt = "No 'number of episodes' (P1113) statement for season {0}"
            report_items.append((False, fmt.format(season_item)))
    ...
    if values:
            ...
            if season_item in grouped_by_season:
                ...
                if n_episodes_from_part_of == expected_number_of_episodes:
                    report_items.append(
                        (
                            True,
                            '''The number of episodes that were 'part of' (P361)
                               season {season_item} matched the number of episodes
                               expected from the 'number of episodes' (P1113) for
                               the season: {n}'''.format(
                                season_item=season_item, n=expected_number_of_episodes)
                        )
                    )
                else:
                    report_items.append(
                        (
                            False,
                            '''The number of episodes that were 'part of' (P361)
                               season {season_item} ({n_part_of}) didn't match
                               the number of episodes expected from the 'number
                               of episodes' (P1113) for the season ({expected})'''.format(
                                season_item=season_item,
                                n_part_of=n_episodes_from_part_of,
                                expected=expected_number_of_episodes)
                        )
                    )
        for value in values:
            if value.get('seriesStatement'):
                if not value['episodeNumber']:
                    report_items.append(
                        (
                            False,
                            '''The episode {episode}'s 'series' (P179) statement
                               linking it to {series_item} lacked a 'series
                               ordinal' (P1545) qualifier'''.format(
                                episode=id_from_item_url(value['episode']),
                ...
                report_items.append(
                    (
                        False,
                        '''The episode {episode} was missing a 'series' (P179)
                           statement linking it to {series_item}'''.format(
                            episode=id_from_item_url(value['episode']),
                            series_item=series_item)
                    )
                )
        ...
        report_items.append(
            (
                False,
                '''Found no episodes with a 'part of' (P361) relationship to any
